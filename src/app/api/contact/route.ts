...
// Basic body shape for validation (kept minimal to avoid extra deps)
type ContactBody = {
	name: string;
	email: string;
	subject?: string;
	...
	try {
		...
		if (!webhookUrl) {
			return NextResponse.json(
				{ ok: false, error: "Missing CONTACT_WEBHOOK" },
				{ status: 500 }
			);
		}
		if (!secret) {
		...
		// Read inbound JSON and validate required user fields
		const inbound = (await req.json()) as Partial<ContactBody>;
		// Minimal validation
		if (!inbound?.name || typeof inbound.name !== "string") {
			return NextResponse.json({ ok: false, error: "Missing name" }, { status: 400 });
		}
		if (!inbound?.email || typeof inbound.email !== "string" || !inbound.email.includes("@")) {
			return NextResponse.json({ ok: false, error: "Invalid email" }, { status: 400 });
		}
		if (!inbound?.message || typeof inbound.message !== "string") {
			return NextResponse.json({ ok: false, error: "Missing message" }, { status: 400 });
		}

		// Derive context (prefer provided values, then headers)
